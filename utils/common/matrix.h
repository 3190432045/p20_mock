...
public:
	...
	_inline matrix3x3( const Vector4D &quaternion )
	{
	...
	// init from angles
	_inline matrix3x3( const Vector &angles )
	{
	...
	_inline int operator == ( const matrix3x3& mat2 ) const
	{
	...
	_inline int operator != ( const matrix3x3& mat2 ) const { return !(*this == mat2 ); }
...
public:
	...
	// init from entity
	_inline matrix3x4( const Vector &origin, const Vector &angles, float scale = 1.0f )
	{
		float	angle, sr, sp, sy, cr, cp, cy;
	...
	// init from entity
	_inline matrix3x4( const Vector &origin, const Vector &angles, const Vector &scale )
	{
		float	angle, sr, sp, sy, cr, cp, cy;
	...
	_inline matrix3x4( const Vector &origin, const Radian &angles, const Vector &scale )
	{
		float	angle, sr, sp, sy, cr, cp, cy;
	...
	_inline matrix3x4( const Vector &origin, const Radian &angles )
	{
		float	angle, sr, sp, sy, cr, cp, cy;
	...
	_inline matrix3x4( const Vector &origin, const Vector4D &quaternion )
	{
		mat[0][0] = 1.0f - 2.0f * (quaternion.y * quaternion.y + quaternion.z * quaternion.z);
		mat[1][0] = 2.0f * (quaternion.x * quaternion.y - quaternion.z * quaternion.w);
		mat[2][0] = 2.0f * (quaternion.x * quaternion.z + quaternion.y * quaternion.w);
	...
	_inline matrix3x4( const matrix3x3 &in )
	{
		mat[0][0] = in[0][0];
		mat[0][1] = in[0][1];
		mat[0][2] = in[0][2];
	...
	_inline int operator == ( const matrix3x4& mat2 ) const
	{
		if( mat[0][0] != mat2[0][0] || mat[0][1] != mat2[0][1] || mat[0][2] != mat2[0][2] )
			return false;
		if( mat[1][0] != mat2[1][0] || mat[1][1] != mat2[1][1] || mat[1][2] != mat2[1][2] )
	...
	_inline int operator != ( const matrix3x4& mat2 ) const { return !(*this == mat2 ); }
...
public:
	...
	// init from entity
	_inline matrix4x4( const Vector &origin, const Vector &angles, float scale = 1.0f )
	{
		float	angle, sr, sp, sy, cr, cp, cy;
	...
	// init from entity
	_inline matrix4x4( const Vector &origin, const Vector &angles, const Vector &scale )
	{
		float	angle, sr, sp, sy, cr, cp, cy;
	...
	// init from quaternion + origin
	_inline matrix4x4( const Vector &origin, const Vector4D &quaternion )
	{
		mat[0][0] = 1.0f - 2.0f * (quaternion.y * quaternion.y + quaternion.z * quaternion.z);
		mat[1][0] = 2.0f * (quaternion.x * quaternion.y - quaternion.z * quaternion.w);
		mat[2][0] = 2.0f * (quaternion.x * quaternion.z + quaternion.y * quaternion.w);
	...
	// init from matrix3x4
	_inline matrix4x4( const matrix3x4 &in )
	{
		mat[0][0] = in[0][0];
		mat[0][1] = in[0][1];
		mat[0][2] = in[0][2];
	...
	_inline int operator == ( const matrix4x4& mat2 ) const
	{
	...
	_inline int operator != ( const matrix4x4& mat2 ) const { return !(*this == mat2 ); }
